...
$(document).ready(function() {
	...
	aRegions.forEach(function(region, index){
					...
					list.forEach(function(summoner, index){
						...
						var SummonerClass = new Summoner(region, summoner.summonerId, '', index);
							...
							if(index <= LIMIT_GENERATE_SUMMONERS){
									...
									let marker = L.marker(
														[randomCoordinate.lat, randomCoordinate.lon], 
														{icon: iconStyle})		   
									.addTo(mymap)

									if(index <= 3){
										marker
										.bindTooltip(SummonerClass.summonerName + " LP: " + SummonerClass.ratedRating, {permanent: true, direction: 'bottom'})
										.openTooltip();
									}
			...
			HyperRollClass.createRankingHyperRollByRegion()
			.done(function(hyperRollList){
				console.log(hyperRollList);
			})

			console.log(RegionClass.getRandomJSONCoordinate());
	...
	// aRegions.forEach(function(region, index){
		...
		// setHTMLRankingRegions(region, index);
	// })

	// let firstLoadRandomRegion = aRegions[getRandomInt(0, aRegions.length)];
	// let randomCoordinate = RegionClass.getRandomJSONCoordinate(firstLoadRandomRegion);

	// console.log("Random zooming " + firstLoadRandomRegion + " in " + randomCoordinate.lat + ", " + randomCoordinate.lon);

	// $("#collapse-" + firstLoadRandomRegion).collapse();

	// mymap.flyTo([randomCoordinate.lat, randomCoordinate.lon], RegionClass.getAreaInfo('zoom'))
...
// 	$.getJSON("json/area/" + region + "_area.json", function(data){
	 
// 	   	L.geoJSON(data, {
// 	   		style: function(){
// 	   			fillColor = getAreaInfo(region, 'theme')
// 	      	return { color: getAreaInfo(region, 'theme'), weight: 1, fillColor: fillColor, fillOpacity: .5 };
// 	    }
...
// 	    console.log("Erro ao drawRegionArea de " + region);
...
// 	return allCoordinates[region][0]['coordinates'][randInt][0]; 
// }
...
// 		return getHyperRollList(region)
// 		.done(function(aHyperRollList){
...
// 					   	"<span class='" + fontRank + " fw-bold float-start text-dark'> " + getAreaInfo(region, "name") + "</span>" + 
// 					    "<span class='badge text-dark text-end'>" + getNumberToK(aHyperRollList.totalRegion) + " Points</span>" +

// 					  "</button>" +
// 					"</h2>" +

// 					"<div class='accordion-collapse collapse' id='collapse-" + region + "' aria-labelledby='heading-" + region + "' data-bs-parent='#accordionRankingRegion'>"+				 
...
// 	let regionRankingTop3 = $("#collapse-" + region + " .card-group");
// 	if(regionRankingTop3.text() <= 0){
...
// 			   		$(regionRankingTop3).append(
						
// 						"<div class='card'>" +
// 							"<img class='card-img-top clickable rounded-circle m-3 " + summoner.summonerId + 
// 							" border border-secondary' src='" + SUMMONER_ICON_PATH + "3667.png' alt='" +  summoner.summonerName + "'/>" +
// 							"<div class='card-body text-center'>" +					
// 								"<h5 class='card-title'><a href='#'>" + summoner.summonerName + "</a></h5>" +
// 									"<p class='card-text'>"  +
// 										"<span class='badge" + region + " rounded-pill fs-6 bg-primary'></span>" +
// 										"<span class='badge bg-primary'><i class='fas fa-bolt'></i> " + summoner.ratedRating + "</span>" +						
// 									"</p>" +
// 								// "<a href='#' class='btn btn-sm btn-primary sumonner-get-more'>More</a>" +
// 							"</div>" +
// 							"<div class='card-footer'>" +
// 								"<small class='text-muted'>Cuz Im alone you see</small>" +
// 							"</div>" +
// 						"</div>" 
// 			   		).click(function(){
						
// 						// getSummonerMatchsStats(region, summoner.id, summoner.puuid);
// 						setHTMLSummonerDetails(region, summoner.id, summoner.puuid);
...
// function setHTMLSummonerLastMatchs(position, region, summonerId, puuid, limit = 3){
...
// 	let listMatchs = $(".summoner-details .card .card-body .list-group");

// 	getAPI(regionPath, path, savePath)
...
// 	console.log("getSummonerMatchsStats");
