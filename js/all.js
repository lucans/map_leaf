...
const LIMIT_GENERATE_SUMMONERS = 15;
...
document.addEventListener('DOMContentLoaded', function() {
	...
	navbarStart(aRegions.sort(compare));
	setHTMLRankingRegions(aRegions.sort(compare));
		...
		hyperrollJsonResult.forEach(function(summoner, index){
			...
			if(index < LIMIT_GENERATE_SUMMONERS){
				...
				$.getJSON("json/summoners/" + summoner.summonerId.toLowerCase() + ".json", function(summonerJsonDetails){
					...
					// setSummonersProfileIcon(region, summonerJsonDetails.id);
...
function setHTMLSummonerDetails(region, summonerId, puuid){
	...
	$.getJSON("json/summoners/" + summonerId.toLowerCase() + ".json", function(summonerJsonResult){
		...
		$(".summoner-details .card").empty().append("<img class='img-rounded clickable rounded card-img-top " + summonerJsonResult.id + "' src='" + SUMMONER_ICON_PATH + summonerJsonResult.profileIconId + ".png' style='heigth: 500px' alt='" +  summonerJsonResult.name + "'/>" +
			"<div class='card-body text-center'>" +
				...
				"<p class='card-text text-warning'><i class='fas fa-bolt text-warning'></i> <span class='badge bg-secondary fs-6'>" + summonerJsonResult.summonerLevel + "</span></p>" +
					...
					"<ul class='list-group' id='summonerMatchsList'></ul>" +
		...
		$(".summoner-details .card .btn").click(function(){			
			setHTMLSummonerLastMatchs('', region, summonerId, puuid);
		})

		$(".summoner-details").fadeIn();
...
function setHTMLRankingRegions(aRegions){

	$("#accordionRankingRegion").fadeIn();
	$("#spinner-loading").hide();
	let tBody = $("#accordionRankingRegion tbody");

	aRegions.forEach(function(region, index){

		tBody.append(
			"<tr class='tr-" + region.name + " clickable'>" +
				"<td><span class='badge text-light m-1 text-uppercase fs-6 " + region.name + "'>" + region.name + "</span>" + getAreaInfo(region.name, "name") + "</td>" +
				"<td></td>" +
				"<td>" + getNumberToK(region.summoners.totalRegion) + "</td>" +
			+ "</tr>").promise()
			.done(function(data) {				
					...
					$(regionRankingTop3).after(
						"<tr class='" + trClassSummoner + "'>" +
							"<td>" +
								...
								"<span><div class='badge p-2 fs-5 bg-secondary'>" + summoner.name + "</div></span>" +							
							"</td>" +		

							"<td>" +
								
								"<div class='badge'><i class='fas " + iconUpDown + " m-2'></i>" + ((diffPosition !== 0 ) ? diffPosition : '') + "</div>" +
								"<div class='badge bg-primary m-1'><i class='fas fa-bolt'></i> " + summonerHyperRoll.ratedRating + " Points</div>" +
							
							"</td>" +
							
							"<td>" +
								"<div class='badge bg-secondary m-1'><i class='fas fa-medal text-warning'></i> " + summonerHyperRoll.wins + " Wins</div>" +							
							"</td>" +	
						"</tr>"	
						).click(function(){
...
function setHTMLSummonerLastMatchs(position, region, summonerId, puuid, limit = 3){
	...
	let listMatchs = $("#summonerMatchsList");
			...
			.done(function(aMatchResult){
					...
					JSON.parse(aMatchResult).info.participants.forEach(function(participant, index){
						...
						if(participant.puuid == puuid){
							...
							console.log(participant.companion.species);

							listMatchs.append("<li class='list-group-item'>" + participant.companion.species + "</li>");
